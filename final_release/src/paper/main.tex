...
\begin{itemize}
    ...
    \item \textbf{Distillation properties}: We find that knowledge distillation preserves dynamical geometry to a near-perfect degree ($\cos \theta = 0.999$), strongly suggesting that interpretability findings \citep{elhage2021mathematical} from teacher models can confidently transfer to their distilled counterparts (Section~4.4).
    
    \item \textbf{Inter-agent communication limits}: Our cross-architecture results ($\cos \theta \approx 0$; Section~4.9) establish fundamental limits on heterogeneous AI agents' ability to develop shared continuous representations without linguistic mediation \citep{moschella2023relative}.
\end{itemize}
